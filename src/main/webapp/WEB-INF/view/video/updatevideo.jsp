		...
		<div class="row">
			...
			<div>
				<form class="form-horizontal"
					action="${pageContext.request.contextPath}/user/video/updatevideo.action">
					...
					<div class="form-group row">
						...
						<div class="col-md-7">
							<input name="video_title" type="text" class="form-control"
								id="input1" placeholder="视频标题" value="${video.video_title}">
						</div>
					</div>
						...
						<div class="col-md-7">
							<select name="speaker_id" class="form-control" id="input2">
								...
								<c:forEach items="${speakernamelist}" var="speaker"
									varStatus="status">
									<option value="${speaker.id}" ${video.speaker_name eq speaker.speaker_name ? "selected" : ""}>${speaker.speaker_name}</option>
								</c:forEach>
							</select>
						</div>
					</div>
						...
						<div class="col-md-7">
							<select name="course_id" class="form-control" id="input2">
								...
								<c:forEach items="${coursenamelist}" var="course"
									varStatus="status">
									<option value="${course.id}" ${video.course_name eq course.course_name ? "selected" : ""}>${course.course_name}</option>
								</c:forEach>
							</select>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-md-7">
							<input name="video_length" type="text" class="form-control"
								id="input4" placeholder="视频时长（秒）" value="${video.video_length}">
						</div>
					</div>
					<div class="form-group">
						<label for="input5" class="col-md-2 col-md-offset-1 control-label">封面图片</label>
						<div class="col-md-7">
							<input name="video_image_url" type="text" class="form-control"
								id="input5" placeholder="视频封面图片地址，网络图片" value="${video.video_image_url}">
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-md-7">
							<input name="video_url" type="text" class="form-control"
								id="input6" placeholder="视频播放地址，网络地址" value="${video.video_url}">
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-md-7">
							<textarea name="video_descr" class="form-control" rows="3">${video.video_descr}</textarea>
						</div>
					</div>
					<div class="form-group">
						<div class="col-md-offset-3 col-md-10">
							...
							<button onclick="javascript:history.go(-1)"
								class="btn btn-default">返回列表</button>
						</div>
					</div>
				</form>
			</div>
